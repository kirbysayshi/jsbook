...
loadBook('demo.jsbook', function(err, res) {
  ...
  function codelet(el, i) {
    if (el.codelet) return el.codelet;

    var c = new Codelet(el);
    el.codelet = c; // Yep. DOM as stateholder.
    c.live();
    c.on('valid', ensureTrailingCodelet);
    ...
    // check immediately if necessary. Defer to ensure all codelets
  ...
  function codelets() {
    return $('.js-codelet .js-in');
  }

  function ensureTrailingCodelet() {
    var clets = codelets();
    var clet = codelet(clets.last()[0]);
    var editor = clet.editor;
    var index = clets.length;
    if (editor.getValue()) {
      $('.js-codelet-list').append(htmlifyBook({
        sections: [{ meta: { index: index }, code: [] }],
        meta: {}
      }));

      codelet(codelets().last()[0], index);
    }
  }
});
...
function htmlifyBook(book) {
  return book.sections.map(function(section, i) {
    return ''
      + '<div class="js-codelet codelet row" data-codeindex="' + (section.meta.index || i) + '">'
